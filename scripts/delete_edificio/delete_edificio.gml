			...
			if edificio_colores[edificio.index, d]{
				...
				for(var c = 0; c < 4; c++){
					...
					if aa >= 0 and bb >= 0 and aa < xsize and bb < ysize and bool_edificio[capa][# aa, bb]{
						...
						if edificio_colores[temp_edificio.index, d] and (in(temp_edificio.index, 1, 2, 3, 4, 5, 6, 7) or in(index, 1, 2, 3, 4, 5, 6, 7)){
				...
				if ds_list_size(next_edificios) >= 2{
					...
					show_debug_message($"  Red: {recurso_nombre[d]}")
					ds_list_remove(redes, red)
